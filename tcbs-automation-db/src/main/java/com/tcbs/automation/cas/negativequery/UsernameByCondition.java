...
public class UsernameByCondition {
  ...
  static String buildQueryGetUsernameByCondition() {
    StringBuilder sql = new StringBuilder("SELECT USERNAME, TCBSID from TCBS_USER")
      .append(" where id in (")
      .append(" select USER_ID from tcbs_application_user where APP_ID =2 and STATUS =:vsdStatus and USER_ID  in( ")
      .append(" select USER_ID from tcbs_application_user where APP_ID =4 and STATUS =:fundStatus )) AND ACCOUNT_STATUS =:accountSTATUS AND ROWNUM=1");
    return sql.toString();
  }
}
