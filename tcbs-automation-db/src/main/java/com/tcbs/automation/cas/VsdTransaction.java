...
public class VsdTransaction {
  ...
  public static VsdTransaction getById(String id) {
    ...
    Query<VsdTransaction> query = casConnection.getSession().createQuery(
  ...
  public static void updateStatusByList(List<String> code105CList, String status) {
    ...
    Query<VsdTransaction> query = CAS.casConnection.getSession().createNativeQuery(
      "UPDATE VSD_TRANSACTION SET STATUS = ?1 WHERE USER_ID IN (SELECT ID FROM TCBS_USER WHERE USERNAME IN (?2))");
  ...
  public static void deleteByList(List<String> code105CList) {
    ...
    Query<VsdTransaction> query = CAS.casConnection.getSession().createNativeQuery(
      "DELETE FROM VSD_TRANSACTION WHERE USER_ID IN (SELECT ID FROM TCBS_USER WHERE USERNAME IN (?1))");
  ...
  @Step
  public static List<VsdTransaction> getByTcbsId(String tcbsId) {
    CAS.casConnection.getSession().clear();
    Query<VsdTransaction> query = CAS.casConnection.getSession().createNativeQuery(
      "SELECT st.* FROM VSD_TRANSACTION st\n" +
        "INNER JOIN TCBS_USER tu \n" +
        "ON st.USER_ID = tu.ID \n" +
        "WHERE tu.TCBSID =:tcbsId\n", VsdTransaction.class);
    query.setParameter("tcbsId", tcbsId);
    return query.getResultList();
  }

  @Step
  public static void deleteByUserId(List<String> tcbsId) {
    casConnection.getSession().clear();
    if (!casConnection.getSession().getTransaction().isActive()) {
      casConnection.getSession().beginTransaction();
    }
    Query<VsdTransaction> query = CAS.casConnection.getSession().createNativeQuery(
      "DELETE FROM VSD_TRANSACTION WHERE USER_ID IN (SELECT ID FROM TCBS_USER WHERE TCBSID IN (?1))");
