...
public class GeneralRating {
  ...
  public Map<String, Object> getByTicker(String ticker) {
    ...
    try {
      List<Map<String, Object>> listPrice = Stoxplus.stoxDbConnection.getSession().createNativeQuery(queryBuilder.toString())
  ...
  public List<Map<String, Object>> getListTickerTop200() {
    ...
    queryBuilder.append(" FROM	stox_tb_ratio r  ");
    queryBuilder.append(" LEFT JOIN stox_tb_Company comp  ");
    queryBuilder.append(" ON r.Ticker = comp.Ticker  ");
    queryBuilder.append(" WHERE	CAST(UpdateDate AS date) = CAST((select MAX(UpdateDate) from stox_tb_ratio) AS date)  ");
    queryBuilder.append(" ORDER BY F5_7 DESC  ");

    try {
      List<Map<String, Object>> listTickers = Stoxplus.stoxDbConnection.getSession().createNativeQuery(queryBuilder.toString())
        .setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE)
        .getResultList();
      return listTickers;
    } catch (Exception ex) {
  ...
  public Map<String, Object> getByIndustry(String ticker) {
    ...
    queryBuilder.append(" AVG(PriceChange1y) as PriceChange1y  ");
    queryBuilder.append(" FROM tbl_idata_price_summary  ");
    queryBuilder.append(" WHERE Ticker IN ( SELECT comp.Ticker FROM stox_tb_Company comp  ");
    queryBuilder.append("               JOIN view_idata_industry vwind  ");
    queryBuilder.append("               ON comp.IndustryID = vwind.IdLevel4  ");
    queryBuilder.append("               where vwind.IdLevel2 = ( SELECT IdLevel2 FROM stox_tb_Company comp  ");
    queryBuilder.append("                                     JOIN view_idata_industry vwind  ");
