...
public class Daily02StockEntity {
  @Step("Get stock from BPM")
  public static List<HashMap<String, Object>> getStockBpm(String etlUpdateDate, String type) {
    ...
    queryBuilder.append("   from RISK_STOCK_DAILY_TRACKING where ETL_UPDATE_DATE = :etlUpdateDate and TYPE = :type ORDER BY CUSTODYCD, SYMBOL");
    try {
  ...
  public static List<HashMap<String, Object>> getListCus() {
    ...
    StringBuilder queryBuilder = new StringBuilder();
    queryBuilder.append("  select * from RISK_CUSTOMER_DAILY_TRACKING where ETL_UPDATE_DATE = (Select max(ETL_UPDATE_DATE) from RISK_CUSTOMER_DAILY_TRACKING) ");
    try {
      return AwsIRis.AwsIRisDbConnection.getSession().createNativeQuery(queryBuilder.toString())
        .setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE).getResultList();
  ...
  @Step("Get max Etl")
  public static List<HashMap<String, Object>> getMaxEtlUpdate(String type) {

    StringBuilder queryBuilder = new StringBuilder();
    queryBuilder.append("  select max(ETL_UPDATE_DATE) ETL_UPDATE_DATE from RISK_CUSTOMER_DAILY_TRACKING where type = :type  ");
    try {
      return AwsIRis.AwsIRisDbConnection.getSession().createNativeQuery(queryBuilder.toString())
        .setParameter("type", type)
        .setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE).getResultList();
