...
public class Daily03CalculateRttEntity {
  @Step("Get stock ")
  public static List<HashMap<String, Object>> getStockBpm(String eltUpdateDate, String type) {
    ...
    queryBuilder.append("  from RISK_STOCK_DAILY_TRACKING ");
    queryBuilder.append("  where ETL_UPDATE_DATE = :etlUpdateDate and type = :type");
    try {
  ...
  @Step("Get customer")
  public static List<HashMap<String, Object>> getResult(String etlUpdateDate, String type) {
    StringBuilder queryBuilder = new StringBuilder();
    queryBuilder.append(" select * from RISK_RTT_CALCULATE_RESULT_DAILY_TRACKING where ETL_UPDATE_DATE = :etlUpdateDate and type = :type   ");
    try {
  ...
  @Step("Get customer")
  public static List<HashMap<String, Object>> getData(String etlUpdateDate, String type) {
    StringBuilder queryBuilder = new StringBuilder();
    ...
    queryBuilder.append(
      " DeltaRTT, ROW_NUMBER() over (PARTITION BY CUSTODYCD order by DeltaRTT asc, SYMBOL asc) as rn, sumDelta,RTT_AFTER_SUM_DELTA,case when RTT_AFTER_SUM_DELTA < MRL_RATE then 'Force Sell' ");
    ...
    queryBuilder.append(" FROM ( ");
    queryBuilder.append(
      " select CUSTODYCD, AFACCTNO,FULL_NAME,MRI_RATE,MRM_RATE,MARGIN_RATE,MRL_RATE,TOTAL_VND,NET_DEBT,SYMBOL, DEBT, CASH,SEASS,sumDelta,TOTAL_ASSETS,TOTAL_ASSETS_BEFORE,TOTAL_ASSETS_BEFORE_BONUS,case when NET_DEBT <> 0 then round(DeltaRtt/abs(NET_DEBT) *100,3) else 0 end  DeltaRTT, round(sumDelta/NET_DEBT*100 + TOTAL_ASSETS_BEFORE/NET_DEBT*100,2) as  RTT_AFTER_SUM_DELTA,round(sumDelta/NET_DEBT*100 + TOTAL_ASSETS_BEFORE_BONUS/NET_DEBT*100,2) as  RTT_AFTER_SUM_DELTA_BONUS ");
    queryBuilder.append(" from ");
    queryBuilder.append(" (select CUSTODYCD, AFACCTNO,FULL_NAME,MRI_RATE,MRM_RATE,MARGIN_RATE,MRL_RATE,TOTAL_VND,NET_DEBT,SYMBOL, DEBT, CASH,SEASS, ");
    ...
    queryBuilder.append(" select * from ( ");
    queryBuilder.append(
      " select CUSTODYCD Cus, SYMBOL,DeltaRTT, rn,sumDelta sumDeltaBonus,case when RTT_AFTER_SUM_DELTA_BONUS <=0 then 0 else RTT_AFTER_SUM_DELTA_BONUS end as RTT_AFTER_SUM_DELTA_BONUS ,STATUS_BONUS,TOTAL_ASSETS_BONUS,case when STATUS_BONUS <> 'Normal' then CEIL(NET_DEBT - TOTAL_ASSETS_BONUS / (MRM_RATE/100)) end as MONEY_ADDED_TOTAL_BONUS,round(TOTAL_ASSETS_BONUS / NET_DEBT * 100, 2) as RTT_AFTER_BONUS ");
    queryBuilder.append(" from ( ");
    queryBuilder.append(" SELECT CUSTODYCD, AFACCTNO,FULL_NAME,MRI_RATE,MRM_RATE, MARGIN_RATE, MRL_RATE,TOTAL_VND,NET_DEBT,SYMBOL, DEBT, CASH,SEASS, ");
    queryBuilder.append(
      " DeltaRTT, ROW_NUMBER() over (PARTITION BY CUSTODYCD order by DeltaRTT asc, SYMBOL asc) as rn, sumDelta,RTT_AFTER_SUM_DELTA_BONUS,case when RTT_AFTER_SUM_DELTA_BONUS < MRL_RATE then 'Force Sell' ");
    ...
    queryBuilder.append(" FROM ( ");
    queryBuilder.append(
      " select CUSTODYCD, AFACCTNO,FULL_NAME,MRI_RATE,MRM_RATE,MARGIN_RATE,MRL_RATE,TOTAL_VND,NET_DEBT,SYMBOL, DEBT, CASH,SEASS,sumDelta,TOTAL_ASSETS_BONUS,case when NET_DEBT <> 0 then round(DeltaRtt/abs(NET_DEBT) *100,3) else 0 end  DeltaRTT, round(sumDelta/NET_DEBT*100 + TOTAL_ASSETS_BEFORE_BONUS/NET_DEBT*100,2) as  RTT_AFTER_SUM_DELTA_BONUS ");
    queryBuilder.append(" from ");
    queryBuilder.append(" (select CUSTODYCD, AFACCTNO,FULL_NAME,MRI_RATE,MRM_RATE,MARGIN_RATE,MRL_RATE,TOTAL_VND,NET_DEBT,SYMBOL, DEBT, CASH,SEASS, ");
