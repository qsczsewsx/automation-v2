...
import com.automation.tca.tcdata.TcDataStox;
    ...
    for (Field field : clazz.getDeclaredFields()) {
      try {
        ...
        if (field.getType().equals(Double.class)) {
          field.set(destination, FormatUtils.parseDouble((fromDb.get(field.getName()))));
        }
      } catch (Exception e) {
    ...
    for (Field field : clazz.getDeclaredFields()) {
      try {
        ...
        if (field.getType().equals(Double.class)) {
          ...
          field.set(destination, FormatUtils.parseLong((fromDb.get(field.getName()))));
        }
      } catch (Exception e) {
        logger.error(e.getMessage(), e.getStackTrace());
      }
