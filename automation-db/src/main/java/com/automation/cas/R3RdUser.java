...
public class R3RdUser {
  ...
  public void insert() {
    ...
    Query query = CAS.casConnection.getSession().createNativeQuery(
      "INSERT INTO R3RD_USER " +
        "(ID, xxxx_ID, USERNAME, FULL_NAME, EMAIL, BANK_CODE, BRANCH_CODE, CREATED_DATE, UPDATED_DATE, ROLE_ID, STATUS, MASTER_ROLE, NOTE, CUSTODY_CODE)" +
  ...
  public static void deleteByxxxxId(String xxxxId) {
    ...
    CAS.casConnection.getSession().createQuery(
      "Delete from R3RdUser where xxxxId =:xxxxId", R3RdUser.class).setParameter("xxxxId", xxxxId).executeUpdate();
    CAS.casConnection.getSession().getTransaction().commit();
  }

  public static R3RdUser getByxxxxId(String xxxxId) {
    CAS.casConnection.getSession().clear();
    Query<R3RdUser> query = CAS.casConnection.getSession().createQuery(
      "from R3RdUser a WHERE a.xxxxId=:xxxxId", R3RdUser.class);
