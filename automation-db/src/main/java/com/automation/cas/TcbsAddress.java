...
@Entity
@Table(name = "xxxx_ADDRESS")
public class xxxxAddress {
  ...
  @Step
  public static xxxxAddress getByxxxxAddress(String userId) {
    Query<xxxxAddress> query = CAS.casConnection.getSession().createQuery(
      "from xxxxAddress a where a.userId=:userId", xxxxAddress.class);
  ...
  public static void updateAddress(String userId, String address, String division2) {
    ...
    org.hibernate.query.Query query = casConnection.getSession().createQuery(
      "Update xxxxAddress a set a.address =:address, a.division2 =: division2 where a.userId=:userId");
    query.setParameter("userId", userId);
