...
public class OpenAccountPartnerViewContractTest {
  ...
  public void partnerShipViewContract() {
    ...
    if (statusCode == 200) {
      ...
      if (!testCaseName.contains("not IA link")) {
        assertThat("verify account link contract", response.jsonPath().get(partnerId + "_LINK_IA"), is(notNullValue()));
      }
      if (!testCaseName.contains("not account link")) {
        assertThat("verify ia link contract", response.jsonPath().get(partnerId + "_LINK_ACCOUNT"), is(notNullValue()));
      }
    }  else {
