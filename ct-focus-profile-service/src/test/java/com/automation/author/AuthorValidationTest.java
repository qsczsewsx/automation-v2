...
import com.automation.authen.AuthenSessionHBTest;
import com.automation.authen.AuthenSessionTest;
import com.automation.authen.AuthenSessionWTTest;
...
public class AuthorValidationTest {
  ...
  private Response callApi() {
    RequestSpecification requestSpecification = given()
            .baseUri(xxxxProfileServiceConfig.AUTHOR_VALIDATION)
  ...
  private String getSotpSign(String token) {
    ...
    authenInfo.put("xxxxId", "0001693997");
    String domain;
    if (HB.equals(otpSource)) {
      domain = xxxxProfileServiceConfig.OTP_HB_DOMAIN;
      authenInfo.put("source", StringUtils.lowerCase(HB));
    } else if (WT.equals(otpSource)) {
      domain = xxxxProfileServiceConfig.OTP_WT_DOMAIN;
    } else {
